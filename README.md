# Hi there 👋, I'm Archana

### Aspiring Software Engineer | Java Developer | AI & Machine Learning Enthusiast

I'm a Computer Science student passionate about building software solutions and exploring Artificial Intelligence, Machine Learning, and Full Stack Development. I enjoy solving coding problems, learning new technologies, and working on real-world projects.

---

## 🚀 About Me

- 🎓 Computer Science Engineering Student
- 💻 Interested in Java, Python, SQL, and Web Development
- 🤖 Exploring Artificial Intelligence and Machine Learning
- 🌱 Currently learning Spring Boot, Hibernate, and Data Structures & Algorithms
- 📫 Reach me at: **archanagottam123@gmail.com**
- 💼 Open to Internship and Full-Time Software Developer opportunities
---

## 🛠️ Tech Stack

### Programming Languages

* Java
* Python
* C
* SQL

### Web Technologies

* HTML
* CSS
* JavaScript
* JSP
* Servlets

### Frameworks & Libraries

* Spring Boot
* Hibernate
* JDBC

### Database

* MySQL

### Tools

* Git
* GitHub
* VS Code
* Eclipse
* IntelliJ IDEA

---

## 📌 Featured Projects

### ❤️ Heart Disease Prediction

* Built a Machine Learning model to predict heart disease.
* Used Python, Pandas, NumPy, Scikit-learn, and Matplotlib.
* Performed data preprocessing, feature selection, model training, and evaluation.

### 🎬 IMDB Movie Analysis

* Analyzed movie datasets using Python.
* Generated insights through data visualization.
* Worked with Pandas, NumPy, and Matplotlib.

### 🌾 Smart Farming Rain-Sensing Roof System

* Developed an automated rain-sensing roof prototype.
* Used sensors and automation concepts to protect crops from unexpected rainfall.

### 🎓 Student Registration System

* Developed using Java, JSP, Servlets, JDBC, Hibernate, and Spring.
* Implemented CRUD operations and database integration.

---

## 📊 GitHub Stats

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=YOUR_GITHUB_USERNAME\&show_icons=true\&theme=tokyonight)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=YOUR_GITHUB_USERNAME\&layout=compact\&theme=tokyonight)

![GitHub Streak](https://streak-stats.demolab.com/?user=YOUR_GITHUB_USERNAME\&theme=tokyonight)

---

## 🏆 Achievements

* 🎓 AI & Machine Learning Projects
* 💼 Internship Experience
* 📚 Continuous Learner
* 🚀 Building real-world software projects

---

## 📈 Currently Working On

* Java Full Stack Development
* Spring Boot Applications
* Data Structures & Algorithms
* Machine Learning Projects
* Open Source Contributions

---

## 🤝 Let's Connect

- 💻 **GitHub:** https://github.com/archanagottam123-png
- 💼 **LinkedIn:** https://www.linkedin.com/in/g-archana-951985361
- 📧 **Email:** archanagottam123@gmail.com

⭐ *"Code. Learn. Build. Repeat."*


